...
void OpenGLProgram::initializeProgramState(OpenGLContext& context, GLuint program,
...
#endif
    {
        ...
        UniformsRecord* const uniformsRecords =
                new UniformsRecord[ lazyInitializationData.bindingUniformInfo.size() ];
        UTILS_NOUNROLL
        for (GLuint binding = 0, n = lazyInitializationData.bindingUniformInfo.size();
                binding < n; binding++) {
